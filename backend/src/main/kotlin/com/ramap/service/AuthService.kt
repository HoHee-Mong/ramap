...
class AuthService(
    ...
    fun signup(request: SignupRequest): AuthResponse {
        ...
        return AuthResponse(token = token, email = request.email, nickname = request.nickname, role = saved.role)
    }
    ...
    fun login(request: LoginRequest): AuthResponse {
        ...
        val token = jwtUtil.generateToken(request.email)
        return AuthResponse(token = token, email = user.email, nickname = user.nickname, role = user.role)
    }

    // 이메일로 유저 정보 조회 (role 포함)
    fun getUserInfo(email: String): UserInfoResponse {
        val user = userRepository.findByEmail(email)
            ?: throw IllegalArgumentException("유저를 찾을 수 없습니다")
        return UserInfoResponse(id = user.id!!, email = user.email, nickname = user.nickname, role = user.role)
    }
}
